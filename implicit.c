...
static void *coalesce(struct heap *h, void *first_block_start) {
  ...
  // printf("coalesce\n");
  void *nxtblk = get_next_block(first_block_start);
  int block_size = get_block_size(first_block_start);
  
  if(is_last_block(h, first_block_start) == 0){
    if (!(block_is_in_use(first_block_start)) && !(block_is_in_use(nxtblk))){
      int nxtSize = get_block_size(nxtblk);
      set_block_header(first_block_start, block_size + nxtSize, 0);
    }
  };
...
static int get_size_to_allocate(int user_size) {
  ...
  /* TO BE COMPLETED BY THE STUDENT. */
  int padding = HEADER_SIZE - (user_size % HEADER_SIZE);
  int block = padding + user_size + (HEADER_SIZE * 2);
  return block;
}
...
{
  /* TO BE COMPLETED BY THE STUDENT. */
  printf("splitandmarkused\n");
  int block_size = get_block_size(block_start);
  int leftover = block_size - needed_size;
    if (leftover >= (HEADER_SIZE * 3)){
      set_block_header(block_start, needed_size, 1);
      set_block_header(get_next_block(block_start), leftover, 0);
      return block_start;
    } else {
...
void myfree(struct heap *h, void *payload) {
  ...
  void *nxtblk;
  void *prvblk;
  startBlock = get_block_start(payload);
  set_block_header(startBlock, get_block_size(startBlock), 0);
  nxtblk = get_next_block(startBlock);
  prvblk = get_previous_block(startBlock);
  if(block_is_in_use(nxtblk) == 0){
    startBlock = coalesce(h, startBlock);
  }
  if(!(is_first_block(h, startBlock))){
    if(!(block_is_in_use(prvblk) == 0)){
      startBlock = coalesce(h, prvblk);
  }}
};
...
void *mymalloc(struct heap *h, unsigned int user_size) {
  ...
  printf("mymalloc\n");
  void *blk;
  int sizeNeeded = get_size_to_allocate(user_size);
  for (blk = h->start; is_within_heap_range(h, blk); blk = get_next_block(blk)){
    printf("in loop\n");
    if (sizeNeeded <= get_block_size(blk) && !(block_is_in_use(blk))){
      return split_and_mark_used(h, blk, sizeNeeded);
    }
  }
  return NULL;
};
